...
const initCounterEl = document.querySelector('.button-init-counter')
const stopCounterEl = document.querySelector('.button-stop-counter')
initCounterEl.addEventListener('click', () => {
  let counter = 1
  
  const countSeconds = setInterval(() => {
    counterContainerEl.textContent = counter++
  }, 1000)
  
  stopCounterEl.addEventListener('click', () => {
    clearInterval(countSeconds)
    counterContainerEl.textContent = 0
  })
})
