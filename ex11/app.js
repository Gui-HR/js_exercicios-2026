...
/*
  ...
  - Você sabe por que isso aconteceu? Pois a variável 'age' possui um escopo de bloco, sendo assim ela não existe fora da função, onde a mesma foi referenciada
*/
...
const car = {
  ...
  getColorsMessage () {
    const lastColor = this.colors[this.colors.length]
    return `O ${this.name} está disponível nas cores ${this.colors.join(', ').replace(`, ${lastColor}`, ` e ${lastColor}`)}`
  }
}
