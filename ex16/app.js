...
elementsInsideDiv.forEach(element => {
  element.addEventListener('click', (event) => {
    ...
    h2El.innerText = `Clicou no ${event.target.nodeName.toLowerCase()}. filho da div.`
    div.insertAdjacentElement('afterend', h2El)
  })
})

div.addEventListener('click', () => {
  h2El.innerText = 'Clicou na div.'
  div.insertAdjacentElement('afterend', h2El)
})
...
h2El.addEventListener('copy', () => console.log('Texto copiado!'))
...
const eggEl = document.querySelector('.egg')


eggEl.addEventListener('mousemove', (event) => eggEl.innerText = `Eixo X: ${event.offsetX} | Eixo Y: ${event.offsetY}`)
