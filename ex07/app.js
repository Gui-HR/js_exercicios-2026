...
const stringMessage = randomStrings.join(', ').replace(`, ${lastValue}`, ` e ${lastValue}`)

for(let i = 0; i < randomValues.length; i++) {
  const randomValue = randomValues[i]
  const type = typeof randomValue

  if(randomStrings.length == 4) {
    console.log(`3 informações sobre o array randomValues:
      - As primeiras 4 strings são ${stringMessage};
      - Até que as primeiras 4 strings fossem iteradas, ${randomBooleans.length} booleans foram iterados;
      - O array foi iterado por ${i} vezes.`)
      
      break
  }

  if(type == 'string') randomStrings.push(randomValue)
  if(type =='boolean') randomBooleans.push(randomValue)
}
