...
const my3FavoriteTVShows = ['Arcane', 'Dark', 'Lost in the space']
const lastSerie = my3FavoriteTVShows[my3FavoriteTVShows.length - 1]
const sentence = `Minhas 3 séries favoritas são: ${my3FavoriteTVShows.join(', ').replace(`, ${lastSerie}`, ` e ${lastSerie}`)}.`
...
const booleanPhrase = `A string que a "typeSentence" armazena tem mais de ${charactersNumber} caracteres? ${String(typeSentence.length >= charactersNumber).replace('t', 'T')}.`

// console.log(booleanPhrase)
...
const falseIndex = falsyValues.indexOf(false)

// console.log(`Todos os ${falsyValues.length} itens do array "falsyValues" são falsy. Inclusive o ${falsyValues[falseIndex]}.`)
...
const crazyOperation = null + 1 // Null é um valor falsy, ou seja, é similar a 0, então a conta fica 0 + 1 = 1
const crazyConversion = Boolean(crazyOperation) // 1 é um valor truthy, com isso é similar a true
...
const agesPhrase = `A soma entre o 1º e o 3º item de "ages" é menor ou igual a 92. Essa afirmação é: ${agesSum <= 92}.`

// console.log(agesPhrase)
...
const includesEight = Boolean(evenNumbers.indexOf(8))
