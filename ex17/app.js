...
const inputEl = formEl.input

formEl.addEventListener('submit', event => {
  event.preventDefault()
  const inputValue = inputEl.value
  const formRegex = /[a-zA-Z0-9]{7,}/

console.log(formRegex.test(inputValue))


  if(formRegex.test(inputValue)) {
...
console.log(regex.test(link.textContent.trim()))
